...
from forecast_bo_runner import deterministic_validation_partition, schedule_hash  # noqa: E402
from joint_progression_ppo_schedule import (  # noqa: E402
    ...
    _merge_global_final_rows,
    _policy_run_fingerprint,
    _selector_row_matches_candidate,
...
def synthetic_observations() -> dict:
    ...
    return {
        ...
        "basis_dim": 5,
        "run_fingerprint": "obs_hash",
        "run_metadata": {},
        "observations": rows,
    }


class JointProgressionPpoTests(unittest.TestCase):
    def test_cli_uses_joint_progression_command_and_rejects_removed_bandit_command(self) -> None:
        parser = build_argparser()

        args = parser.parse_args(
            [
                "run-forecast-joint-progression-ppo",
                "--out-root",
                "outputs/future_work/joint_progression_ppo/demo",
                "--datasets",
                "synthetic",
                "--target-nfes",
                "10",
        ...
        self.assertEqual(args.command, "run-forecast-joint-progression-ppo")
        self.assertEqual(args.lambda_kl, 0.05)
        self.assertEqual(args.ppo_updates, 20)
        with redirect_stderr(StringIO()), self.assertRaises(SystemExit):
            parser.parse_args(
                [
                    "run-forecast-ppo-bandit",
                    "--out-root",
                    "outputs/future_work/removed_policy/demo",
                    "--datasets",
                    "synthetic",
                    "--target-nfes",
                    "10",
    ...
    def test_bo_elite_calibration_tolerates_tiny_negative_roundoff_kl(self) -> None:
        observations = synthetic_observations()
        observations["observations"][0]["kl_to_reference"] = -8e-17

        calibration = calibrate_from_bo_elites(synthetic_reference(), observations, elite_fraction=0.2)

        self.assertGreater(calibration["n_elites"], 0)
    ...
    def test_matching_bo_solver_dir_requires_explicit_strict_cache_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json(
                solver_dir / "confirmation_rows.json",
                {
                    "run_fingerprint": "confirm_hash",
                    "run_metadata": {"observation_run_fingerprint": "obs_hash"},
                    "rows": [synthetic_observations()["observations"][0]],
                },
            )

            self.assertIsNone(find_matching_bo_solver_dir(workspace_root=root, dataset="synthetic", target_nfe=10, solver_key="euler"))
    ...
    def test_matching_bo_solver_dir_rejects_metadata_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            observations["basis_dim"] = 5
            observations["run_fingerprint"] = "obs_hash"
            observations["run_metadata"] = {"lambda_kl": 0.05, "bo_batch_size": 2}
            ...
            write_json(
                ...
                {
                    "run_fingerprint": "confirm_hash",
                    "run_metadata": {"observation_run_fingerprint": "obs_hash", "lambda_kl": 0.05, "bo_batch_size": 2},
                    "rows": [observations["observations"][0]],
                },
            )
    ...
    def test_matching_bo_solver_dir_rejects_theta_dimension_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run = root / "outputs" / "future_work" / "bo_schedule_search" / "run_a"
            solver_dir = run / "euler"
            observations = synthetic_observations()
            observations["observations"][0]["theta"] = [0.0] * 6
            observations["basis_kind"] = "tilt_curvature_three_local_bumps"
            write_json(run / "run_config.json", {"dataset": "synthetic", "target_nfe": 10, "basis_kind": "tilt_curvature_three_local_bumps"})
            write_json(solver_dir / "reference_schedule.json", synthetic_reference())
            write_json(solver_dir / "observations.json", observations)
            write_json(solver_dir / "best_schedule.json", {"best_observation": observations["observations"][0]})
            write_json(solver_dir / "validation_split.json", {"calibration_indices": [0], "bo_validation_indices": [1]})
    ...
    def test_merge_global_final_rows_preserves_previous_no_resume_cells(self) -> None:
        first = {
            "dataset": "synthetic",
            "target_nfe": 10,
            "solver_key": "euler",
            "schedule_key": "joint_progression_ppo_best",
            "seed": 0,
            "avg_relative_ratio": 0.9,
        }
        second = dict(first, solver_key="heun", avg_relative_ratio=0.8)

        merged = _merge_global_final_rows([first], [second])

        self.assertEqual(len(merged), 2)
        self.assertEqual({row["solver_key"] for row in merged}, {"euler", "heun"})

    def test_resume_training_rows_drop_partial_updates_and_selector_rows(self) -> None:
        rows = [
            {"split": "ppo_train_70pct", "update": 0, "sample_id": 0, "run_fingerprint": "current"},
            {"split": "ppo_train_70pct", "update": 0, "sample_id": 1, "run_fingerprint": "current"},
            {"split": "ppo_train_70pct", "update": 1, "sample_id": 0, "run_fingerprint": "current"},
            {"split": "ppo_train_70pct", "update": 2, "sample_id": 0, "run_fingerprint": "stale"},
            {"split": "ppo_train_70pct", "update": 2, "sample_id": 1, "run_fingerprint": "stale"},
            {"split": "ppo_selector_30pct", "update": 0, "sample_id": 0},
        ]

        kept, completed = _complete_train_rows_for_resume(rows, batch_size=2, run_fingerprint="current")

        self.assertEqual(completed, 1)
        self.assertEqual(len(kept), 2)
        self.assertTrue(all(row["update"] == 0 for row in kept))

    def test_selector_rows_require_hash_and_run_fingerprint(self) -> None:
        schedule = [0.0, 0.5, 1.0]
        record = {"schedule_grid": schedule}
        row = {
            ...
            "eval_examples": 3,
            "num_eval_samples": 5,
            "selector_indices_hash": "selector_hash",
        ...
        stale = {key: value for key, value in row.items() if key != "run_fingerprint"}
        self.assertFalse(
            _selector_row_matches_candidate(
                stale,
                candidate_id="candidate",
    ...
    def test_policy_run_fingerprint_changes_with_optimizer_and_bo_settings(self) -> None:
        import argparse
        ...
        checkpoint = {"checkpoint_id": "ckpt", "train_steps": 20000}
        split_payload = {
            "calibration_windows": 2,
            "selector_windows": 2,
            "calibration_indices": [0, 1],
            "selector_indices": [2, 3],
        }
        calibration = {"beta_ref": 0.05, "hard_kl_cap": 0.1, "policy_mu": [0.0] * 5, "policy_std": [0.1] * 5}
        first = _policy_run_fingerprint(
            base,
            ...
            bo_observation_fingerprint="bo_hash",
        )
        changed = argparse.Namespace(**vars(base))
        changed.lambda_kl = 0.0
        second = _policy_run_fingerprint(
            changed,
            checkpoint=checkpoint,
            dataset="synthetic",
            target_nfe=10,
    ...
    def test_removed_bandit_final_row_is_not_reused(self) -> None:
        import argparse
        ...
        removed_row = {
            ...
            "checkpoint_id": "ckpt",
            "schedule_grid": schedule,
            "schedule_hash": "bad_hash_should_be_recomputed_from_grid",
            "test_indices_hash": test_hash,
        }
        ...
        removed_row["schedule_hash"] = schedule_hash(schedule)
        self.assertIsNone(
            _matching_cached_row_for_ppo(
                [removed_row],
                args=argparse.Namespace(num_eval_samples=5),
    ...
    def test_final_row_without_test_hash_is_rejected(self) -> None:
        ...
        row = {
            ...
            "schedule_hash": schedule_hash([0.0, 0.5, 1.0]),
        }
    ...
    def test_global_final_summary_keeps_dataset_and_nfe_distinct(self) -> None:
        ...
        self.assertIn("joint_progression_ppo_best", PPO_COMPARISON_SCHEDULES)
