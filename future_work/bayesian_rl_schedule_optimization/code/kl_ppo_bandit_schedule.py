...
from residual_parameterization import DEFAULT_BASIS_KIND
...
def _policy_run_fingerprint(args: argparse.Namespace, *, checkpoint: Mapping[str, Any], dataset: str, target_nfe: int, solver_key: str, reference: Mapping[str, Any], split_payload: Mapping[str, Any]) -> str:
    return _stable_hash(
        {
            "artifact": "ppo_bandit_policy_run_fingerprint_v1",
            "dataset": str(dataset),
...
def _cell_args(args: argparse.Namespace, *, dataset: str, target_nfe: int, solver_key: str, out_root: Path) -> argparse.Namespace:
    return argparse.Namespace(
        ...
        baseline_cache_roots=str(args.baseline_cache_roots),
...
def _ensure_bo_warmstart(
    *,
    ...
    cache_roots = parse_csv(str(args.bo_cache_roots))
    existing = find_matching_bo_solver_dir(
...
def _run_ppo_training_for_cell(
    *,
    ...
    write_json(calibration, cell_out / "ppo_bandit_calibration.json")
    ...
    split_payload = {
        "artifact": "ppo_bandit_validation_split_v1",
    ...
    write_json(split_payload, cell_out / "ppo_bandit_split.json")
    ...
    write_json(
        {
            "artifact": "ppo_bandit_uniform_baselines_v1",
            "train_uniform": train_uniform,
            "selector_uniform": selector_uniform,
        },
        cell_out / "ppo_bandit_uniform_baselines.json",
    )
    ...
    best_path = cell_out / "ppo_bandit_best_selector.json"
    selector_metadata = {
        "artifact": "ppo_bandit_selector_metadata_v1",
        "run_fingerprint": run_fingerprint,
        "train_rows_hash": _train_rows_hash(train_rows),
    ...
    if best_selector is None:
        ...
        current_mean = theta_to_checked_schedule(q_ref, policy.mu.detach().cpu().numpy(), basis=basis)
        ...
        best_selector = {
            "artifact": "ppo_bandit_best_selector_v1",
    ...
    _write_csv_rows(summary_rows, cell_out / "ppo_bandit_summary.csv")
    return {
        "cell_out": cell_out,
        "reference": reference,
...
def _final_rows_for_cell(
    *,
    ...
    for schedule_key in PPO_COMPARISON_SCHEDULES:
        ...
        elif schedule_key == "ppo_bandit_best":
    ...
    for seed in parse_int_csv(str(args.final_test_seeds)):
        ...
        if uniform_key not in by_key:
            cached = _matching_cached_row_for_ppo(
                cached_rows,
                args=args,
                checkpoint=checkpoint,
                dataset=str(dataset),
    ...
    write_json({"artifact": "ppo_bandit_final_comparison_rows_v1", "rows": out}, final_path)
    return out
...
def run_forecast_ppo_bandit(args: argparse.Namespace) -> Dict[str, Any]:
    import torch
    ...
    run_config = {
        "artifact": "ppo_bandit_run_config_v1",
    ...
    cached_rows = _load_cached_final_rows(all_final_cache_roots(workspace_root, parse_csv(str(args.baseline_cache_roots))))
    ...
    for dataset in datasets:
        ...
        for target_nfe in target_nfes:
            for solver_idx, solver_key in enumerate(solvers):
                ...
                if not bool(args.skip_final_comparison):
                    all_final_rows = _final_rows_for_cell(
                        args=args,
                        core=core,
                        checkpoint=checkpoint,
                        cell_result=result,
                ...
                summary = {
                    ...
                    "solver_key": str(solver_key),
                    "cell_out": str(cdir),
                    "bo_solver_dir": str(bo_solver_dir),
...
def add_run_forecast_ppo_bandit_parser(subparsers: Any) -> None:
    run = subparsers.add_parser("run-forecast-ppo-bandit", help="Run forecast bandit KL-PPO schedule search.")
    ...
    run.add_argument("--bo-cache-roots", type=str, default="")
    run.add_argument("--baseline-cache-roots", type=str, default="")
    run.add_argument("--device", type=str, default="cuda")
    run.add_argument("--skip-final-comparison", action="store_true")
