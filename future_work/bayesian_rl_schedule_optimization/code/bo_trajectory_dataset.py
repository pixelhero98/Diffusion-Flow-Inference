...
def calibrate_from_bo_elites(
    ...
    *,
    ...
    elite_kls = [float(row.get("kl_to_reference", 0.0)) for row in elites]
    ...
    return {
        "artifact": "ppo_bandit_bo_elite_calibration_v1",
...
def find_matching_bo_solver_dir(
    *,
    ...
    search_root = root / "outputs" / "future_work" / "bo_schedule_search"
    ...
    for candidate in candidates:
        ...
        if checkpoint_id is not None:
            config_checkpoint = run_config.get("checkpoint_id", reference.get("checkpoint_id"))
            if config_checkpoint is None or str(config_checkpoint) != str(checkpoint_id):
                continue
        if train_steps is not None:
            config_steps = run_config.get("otflow_train_steps", run_config.get("train_steps", reference.get("train_steps")))
            if config_steps is None:
                checkpoint_text = str(run_config.get("checkpoint_id", reference.get("checkpoint_id", ""))).lower()
                train_label = f"{int(train_steps) // 1000}k" if int(train_steps) % 1000 == 0 else str(int(train_steps))
                if train_label not in checkpoint_text:
                    continue
            elif int(config_steps) != int(train_steps):
                continue
        if bo_budget is not None:
            config_budget = run_config.get("bo_budget")
        ...
        if calibration_fraction is not None:
            config_fraction = run_config.get("calibration_fraction")
        ...
        if reference_macro_factor is not None:
            config_factor = run_config.get("reference_macro_factor", reference.get("reference_macro_factor"))
            if config_factor is None or abs(float(config_factor) - float(reference_macro_factor)) > 1e-12:
                continue
        if num_eval_samples is not None:
            config_samples = run_config.get("num_eval_samples", observations.get("uniform_validation_metrics", {}).get("num_eval_samples"))
...
def all_final_cache_roots(workspace_root: str | Path, extra_roots: Sequence[str | Path] = ()) -> List[Path]:
    ...
    search_root = root / "outputs" / "future_work" / "bo_schedule_search"
    if search_root.exists():
        out.extend(path for path in search_root.iterdir() if path.is_dir())
