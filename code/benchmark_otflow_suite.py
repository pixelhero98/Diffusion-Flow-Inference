...
from otflow_experiment_plan import experiment_plan_by_key
...
def _make_base_args(cli_args: argparse.Namespace, plan: DatasetPlan, history_len: int) -> argparse.Namespace:
    ...
    experiment_spec = experiment_plan_by_key().get(str(plan.dataset))
    future_block_len = (
        int(experiment_spec.future_block_len)
        if experiment_spec is not None and str(plan.dataset) == SLEEP_EDF_DATASET_KEY
        else 1
    )
    rollout_mode = "non_ar" if int(future_block_len) > 1 else "autoregressive"
    ...
    return argparse.Namespace(
        ...
        future_block_len=int(future_block_len),
