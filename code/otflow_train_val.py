...
def _validate_model_dataset_support(model_name: str, cfg: LOBConfig) -> None:
    baseline_without_condition_support = {"trades", "cgan", "timecausalvae", "timegan", "kovae"}
    if model_name not in baseline_without_condition_support:
        return
    if int(getattr(cfg.model, "cond_dim", 0) or 0) > 0:
        raise ValueError(f"{model_name} does not support conditional-generation inputs.")
    if bool(getattr(cfg.model, "use_time_features", False)) or bool(getattr(cfg.model, "use_time_gaps", False)):
        raise ValueError(f"{model_name} does not support time-feature context inputs.")
