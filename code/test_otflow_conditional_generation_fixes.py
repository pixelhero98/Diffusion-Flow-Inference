...
from fm_backbone_registry import BACKBONE_NAME_OTFLOW, LOB_FAMILY, materialize_backbone_manifest
from otflow_datasets import build_dataset_splits_from_arrays
from otflow_evaluation_support import load_lob_checkpoint_splits
from otflow_medical_datasets import prepare_sleep_edf_dataset
from otflow_model import OTFlow
from otflow_train_val import _parse_batch, select_eval_window_starts, train_loop
...
class ConditionalGenerationFixesTest(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "future_block_len": int(cfg.model.future_block_len),
                        "split_stats": {"cond_dim": 5, "history_len": int(cfg.history_len)},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_readiness_manifest_marks_conditional_checkpoint_without_conditional_state_invalid(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            torch.save({"cfg": cfg.to_dict(), "model_state": model.state_dict()}, artifact_dir / "model.pt")
            (artifact_dir / "checkpoint_metadata.json").write_text(
                json.dumps({"checkpoint_id": "sleep_bad", "train_steps": 20000, "split_stats": {"cond_dim": 5}}),
                encoding="utf-8",
            )
        ...
        self.assertIn("split_stats.cond_dim=5", sleep_rows[0]["compatibility_error"])
    ...
    def test_unconditional_baselines_reject_conditional_training_data(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "does not support conditional-generation"):
