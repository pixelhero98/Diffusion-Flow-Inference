...
class OTFlowCoreCleanupTest(unittest.TestCase):
    ...
    def test_checkpoint_loader_filters_deprecated_otflow_keys(self) -> None:
        torch.manual_seed(3)
        cfg = self._cfg(use_minibatch_ot=True)
        model = OTFlow(cfg)
        ...
        self.assertTrue(loaded_cfg.fm.use_minibatch_ot)
        self.assertFalse(hasattr(loaded_cfg.fm, "lambda_imbalance"))
